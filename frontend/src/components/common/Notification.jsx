...
const Notification = () => {
   ...
   const handleAllMarkRead = async () => {
      try {
         const res = await axiosInstance.post('/api/user/getallnotification', { userId: user._id }, {
            headers: {
   ...
   const handledeleteAllMark = async () => {
      try {
         const res = await axiosInstance.post('/api/user/deleteallnotification', { userId: user._id }, {
            headers: {
               Authorization: `Bearer ${localStorage.getItem('token')}`,
            },
         })
